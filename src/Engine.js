...
export class Engine {
    ...
    constructor(extensionRootUri, solutionfilename, session, csrf) {
        ...
        if (session && csrf) {
            ...
            this.apiEntry
                .whoami()
                .then((wai) => (this.authenticated = wai.isSignedIn));
        } else {
    ...
    async createSolutionFile() {
        ...
        const selectedLanguage = this.problemData.codeSnippets.filter(
            (cs) => cs.lang == "JavaScript",
        );
        ...
        try {
            ...
            vscode.window.showErrorMessage(
                `Error creating solution file: ${error.message}`,
            );
        }
    }
...
function clampFileName(problem) {
    return (
        problem.title.slice(0, fileNameLength).replace(" ", "") +
