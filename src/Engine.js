...
export class Engine {
	...
	async createTestsFile(problemPath) {
		...
		try {
			...
			vscode.window.showErrorMessage(`API is not responding??? idfk what this error is: ${e.message}`)
		}
	 ...
	 */
	async generateTests() {
		const ai = new GoogleGenAI({apiKey: "AIzaSyA_qzed6vWfwOKDWjfxk7O8YVqZzmWFpjI"});

		const response = await ai.models.generateContent({
			model: "gemini-3-flash-preview",
			contents: `You are a software engineer trying to create test cases in ` 
					+ this.currentLanguage + ` for a leetcode problem to test all general and edge cases. 
					This is the problem description, generate me test cases for the following problem, 
					and DO NOT respond with anything BUT the test cases. 
					Please format each test case with ONLY the parameter values. 
					No name delclarations, just one test case parameter, new line, then the next: ` 
